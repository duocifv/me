...
//       "Khuyến_mãi": update ? update["Khuyến mãi"] : "-",
//       "Tình_trạng": update ? update["Tình trạng"] : "-",
//       "Thực_đơn": update ? update["Thực đơn"] : "-"
//     };
//   });
...
function doGet(e) {
  ...
  if (action === "rooms") {
    ...
    if (!rows)
      return _json({ success: false, error: "Sheet 'RoomList' not found" });
  ...
  if (action === "services") {
    ...
    if (!rows)
      return _json({ success: false, error: "Sheet 'ServiceList' not found" });
  ...
  if (action === "hotel") {
    ...
    if (info === null)
      return _json({ success: false, error: "Sheet 'HotelInfo' not found" });
  ...
  if (action === "bookings") {
    ...
    if (!rows)
      return _json({ success: false, error: "Sheet 'Bookings' not found" });
  ...
  if (action === "roomStatus") {
    ...
    if (!rooms || !updates) {
      return _json({
        success: false,
        error: "Missing RoomList or DailyUpdate",
      });
    }
    ...
    const merged = rooms.map((room) => {
      const update =
        todayUpdates.find((u) => u["Loại phòng"] === room["Loại phòng"]) || {};
      return {
        ...room,
        Giá: update["Giá"] || "-",
        Khuyến_mãi: update["Khuyến mãi"] || "-",
        Tình_trạng: update["Tình trạng"] || "-",
        Thực_đơn: update["Thực đơn"] || update["Thực đơn hôm nay"] || "-",
      };
    });
  ...
  return _json({
    success: false,
    error:
      "Invalid action. Use ?action=rooms | services | hotel | bookings | roomStatus",
  });
}
...
//         imageUrl = saveImage(params.image_base64, 'post_' + newId + '.png') || '';
//       }
//       const newRow = [
//         newId,
//         params.title || "",
...
//           if (params.image_base64) {
//             imageUrl = saveImage(params.image_base64, 'post_' + id + '.png') || imageUrl;
...
function testUpload() {
  ...
  const sampleBase64 =
    "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8HwQACfsD/QqTkN4AAAAASUVORK5CYII=";
