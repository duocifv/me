...
import { BookingDto, BookingSchema, PartialBooking } from './dto/booking.dto';
...
export class ChatService {
  ...
  private provisional = new Map<string, ProvisionalRecord>(); // key = sessionId
  ...
  async handleMessage(sessionId: string, message: string, chatHistory: any[]) {
    const aiReply = await this.gemini.chatHotel(message, chatHistory);

    const parsed = await this.gemini
      .parseBooking(message, chatHistory)
      .catch(() => ({}) as PartialBooking);
    const merged = this.mergeWithFallback(sessionId, parsed, message);

    let rec = this.provisional.get(sessionId);
    if (!rec) {
      rec = {
        sessionId,
        booking: merged,
      ...
      this.provisional.set(sessionId, rec);
    } else {
      rec.booking = { ...rec.booking, ...merged };
      rec.expiresAt = Date.now() + this.TTL;
      this.provisional.set(sessionId, rec);
    }

    if (this.isComplete(rec.booking)) {
      rec.idempotencyKey = this.computeIdempotencyKey(rec.booking);
      const confirmMessage = this.buildConfirmMessage(rec.booking);
      return { aiReply, confirmRequired: true, confirmMessage };
    }

    return { aiReply, confirmRequired: false };
  }
  ...
  private buildConfirmMessage(b: PartialBooking) {
    const name = b.name || '-';
    const phone = b.phone || '-';
    const email = b.email || '-';
    const room = b.room || '-';
    ...
    return `Xác nhận đặt phòng: ${room} từ ${checkin} đến ${checkout} cho ${guests} khách. Tên: ${name}, SĐT: ${phone}, Email: ${email}.`;
  }
