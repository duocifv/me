...
export class CreateMigration1746502126338 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`users\` CHANGE \`lastLoginAt\` \`lastLoginAt\` timestamp NULL`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`user_roles\` ADD \`role_id\` varchar(36) NOT NULL`,
    );
    await queryRunner.query(`ALTER TABLE \`user_roles\` DROP PRIMARY KEY`);
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`user_roles\` ADD \`role_id\` int NOT NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX \`IDX_b23c65e50a758245a33ee35fda\` ON \`user_roles\` (\`role_id\`)`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`users\` CHANGE \`lastLoginAt\` \`lastLoginAt\` timestamp NULL DEFAULT 'NULL'`,
    );
    await queryRunner.query(
