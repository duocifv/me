...
export class CreateMigration1745980703597 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`refresh_token\` (\`id\` varchar(255) NOT NULL, \`expiresAt\` timestamp NOT NULL, \`usageCount\` int NOT NULL DEFAULT '0', \`revoked\` tinyint NOT NULL DEFAULT 0, \`ipAddress\` varchar(255) NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`userId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`permissions\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` enum ('MANAGE_USERS', 'MANAGE_PRODUCTS', 'MANAGE_ORDERS', 'VIEW_PRODUCTS', 'PLACE_ORDER', 'VIEW_ORDERS', 'ACCESS_DASHBOARD') NOT NULL, UNIQUE INDEX \`IDX_48ce552495d14eae9b187bb671\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
