...
export class CreateMigration1745832064102 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`refresh_token\` ADD \`id\` varchar(255) NOT NULL PRIMARY KEY`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`refresh_token\` CHANGE \`usageCount\` \`usageCount\` int NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`refresh_token\` CHANGE \`ipAddress\` \`ipAddress\` varchar(255) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`refresh_token\` CHANGE \`userId\` \`userId\` varchar(36) NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`refresh_token\` CHANGE \`userId\` \`userId\` varchar(36) NULL DEFAULT 'NULL'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`refresh_token\` CHANGE \`ipAddress\` \`ipAddress\` varchar(255) NULL DEFAULT 'NULL'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`refresh_token\` CHANGE \`usageCount\` \`usageCount\` int NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`refresh_token\` DROP COLUMN \`expiresAt\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`refresh_token\` ADD \`expiresAt\` datetime NOT NULL`,
    );
